...
namespace json {
    ...
    Document JsonReader::ProcessRequests(std::istream &input) {
    ...
    void JsonReader::ProcessBaseBusRequests(const std::vector<json::Node> &buses_requests) {
        std::string name;
    ...
    void JsonReader::ProcessBaseStopRequests(const std::vector<json::Node> &stops_requests) {
        std::string name;
    ...
    Document JsonReader::ProcessStateRequests(const std::vector<Node> &state_stops_requests) {
        Array arr;
        for (const auto &request : state_stops_requests) {
            ...
            if (request_data.at("type"s) == "Stop"s) {
                ...
                std::set<std::string> buses_on_stop = rh_.GetBusesOnStop(
